...
Deno.test(
    testNames.testRegisterUser2,
    async () => {
        const requestOption = {
            method: "POST",
            body: JSON.stringify(data.user2),
            headers: {
                'Content-Type': 'application/json',
            }
        }
...
Deno.test(
    ...
    async () => {
        ...
        const respText = await resp.text()
        const respJson = JSON.parse(respText)
        localStorage.setItem("accessToken2", respJson.access)
        t.assertEquals(resp.status, 200);
    }
